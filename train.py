...
class Model(ModelDesc):
    def _get_inputs(self):
        return [InputDesc(tf.uint8, [None, None, None, cfg.frame_extract_pattern.count(1)], 'input'),
                InputDesc(tf.int32, [None], 'label')]
    
    def _build_graph(self, input_vars):
        frames, label = input_vars
        split_frames = tf.split(frames, cfg.frame_extract_pattern.count(1), -1)
        tf.summary.image('input-image', tf.concat([split_frames[0], split_frames[1], split_frames[2], split_frames[3], split_frames[4]], axis=2),  max_outputs=10)
        # frames = tf.identity(frames, name="NETWORK_INPUT")

        l = tf.cast(frames, tf.float32) / 255.0 - 1
    ...
    return TrainConfig(
        ...
        callbacks = [
            ...
            ScheduledHyperParamSetter('learning_rate',
                                      [(0, 1e-3), (30, 3e-3), (60, 1e-3), (85, 1e-4), (95, 1e-5)]),
